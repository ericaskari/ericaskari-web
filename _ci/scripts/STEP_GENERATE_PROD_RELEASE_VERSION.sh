...
if [ "$LAST_DEV_RELEASE" = 'null' ]; then
    printf "No dev releases after last prod release. Using last prod release.\n\n"

    LAST_DEV_RELEASE="$LAST_PROD_RELEASE"
...
sed -i='backup' -e "s|$(cat ./package.json | jq -r '.version')|${LAST_PROD_RELEASE}|" ./package.json

NEW_PACKAGE_JSON_VERSION="$(npm version minor --allow-same-version --git-tag-version false)"
