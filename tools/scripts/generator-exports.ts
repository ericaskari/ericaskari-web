...
import { generateDatabaseEntityRepositoryServices, generateExceptionListExport, generateExportFile, log } from './generator-exports-core';
...
(async () => {
    ...
    generateDatabaseEntityRepositoryServices({
        autogeneratedFile: resolve(process.cwd(), 'libs/api-database/src/repositories.ts'),
        formatter: prettierFormat,
        entitiesDir: resolve(process.cwd(), 'libs/api-database/src/entities'),
        repositoriesDir: resolve(process.cwd(), 'libs/api-database/src/repositories')
    });

    generateExportFile({
        exportFilePath: resolve(process.cwd(), `libs/api-database/src/repositories.ts`),
        staticImports: [],
        dirPath: resolve(process.cwd(), 'libs/api-database/src/repositories'),
        relativeImportBuilder: ({ className, fileNameWithoutType }) =>
            `import { ${className} } from './repositories/${fileNameWithoutType}'`,
        extend: (data) => {
            return [
                data.map(({ className }) => `export { ${className} };`).join('\n'),
                `export const repositories = [ ${data.map(({ className }) => className).join(', ')} ];`
            ];
        },
        classNameFinder: ({ fileData }) => getClassNameFromFileData(fileData),
        prettier: prettierFormat
